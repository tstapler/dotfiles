...
def tag_mod(mod_id, name, mapping):
    # Preserve existing URL if updating name
    info = get_mod_info(mod_id, mapping)
    info["name"] = name
    mapping[mod_id] = info
        ...
        if uplugin_files:
            try:
                with open(uplugin_files[0], 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if friendly_name:
                        # Normalize mapping entry
                        current = get_mod_info(mod_id, mapping)
                        
                        # Update if new info or previously unknown
...
def show_status():
    ...
    if os.path.exists(GUS_PATH):
        ...
        print("\n[Display Settings]")
        res_x = parse_ini_value(gus_content, 'ResolutionSizeX')
        res_y = parse_ini_value(gus_content, 'ResolutionSizeY')
        print(f"  Resolution:         {res_x}x{res_y}")
        print(f"  Fullscreen Mode:    {parse_ini_value(gus_content, 'FullscreenMode')} (0=Windowed, 1=WindowedFullscreen, 2=Fullscreen)")
    else:
...
def main():
    ...
    list_parser.add_argument("--all", action="store_true", help="Show all installed mods, including disabled ones")
    
    subparsers.add_parser("apply", help="Apply configurations")
    subparsers.add_parser("scan", help="Scan local mod files for names/URLs")
