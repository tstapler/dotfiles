    ...
    def get_hosts(self):
        ...
        **Returns**
        ...
        args = [
            command,
            "-n",
            # -I: use ICMP ECHO probes instead of UDP. Docker's bridge NAT
            # rewrites UDP source ports, which breaks matching returned
            # ICMP TTL-exceeded packets back to the probing socket beyond
            # the gateway hop; ICMP echo doesn't have that problem, so this
            # is required for traceroute to work correctly from inside a
            # container.
            "-I",
            # -w wait time seconds
            "-w1",
