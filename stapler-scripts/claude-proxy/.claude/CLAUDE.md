...
- [x] **Investigate CacheAligner concept** — Resolved 2026-08-10 by reading `claw-compactor` (`github.com/open-compress/claw-compactor`, MIT) `main` directly: `FusionEngine`'s default pipeline already includes `QuantumLock` (`scripts/lib/fusion/quantum_lock.py`, order=3, runs before every other stage) — it detects dynamic fragments (dates, UUIDs, tokens, timestamps) in system messages, replaces them with stable placeholders, and appends the real values in a delimited block at the *end* of the message, so the cache-keyed prefix stays stable across requests. This proxy calls `FusionEngine(enable_rewind=True, aggressive=True)` with no stage filtering (`compactor.py:59`), so QuantumLock is already active in production — no action needed here. Separately, claw-compactor also ships a standalone `CachePrefixManager` (`scripts/lib/fusion/cache_prefix.py`, v8) that computes the longest stable prompt-cache prefix across compaction rounds — this one is *not* invoked by `compactor.py` (it's part of an opt-in session-compaction layer, not the per-request `FusionPipeline`), so it's a legitimate future addition if session-level compaction is ever added here. The Rust rewrite (`claude-proxy-rs`) has neither piece yet — tracked in its own `CLAUDE.md` "Open gaps" list, item 1.
  - Reference: [claw-compactor](https://github.com/open-compress/claw-compactor)
