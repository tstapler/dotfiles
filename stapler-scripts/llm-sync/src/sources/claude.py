...
# Claude Code uses .claude/skills/<name>/SKILL.md and .claude/commands/<name>.md
...
class ClaudeSource(SyncSource, SyncTarget):
    def __init__(
        ...
        self.agents_dir = agents_dir or (
            local_claude / "agents"
            if (local_claude / "agents").exists()
            else Path.home() / ".claude" / "agents"
        )
        self.skills_dir = skills_dir or (
            local_claude / "skills"
            if (local_claude / "skills").exists()
            else Path.home() / ".claude" / "skills"
        )
        self.commands_dir = commands_dir or (
            local_claude / "commands"
            if (local_claude / "commands").exists()
            else Path.home() / ".claude" / "commands"
        )
        if local_claude.exists():
            console.print(
                f"[dim]Using local project-specific .claude directory: {local_claude}[/dim]"
            )
