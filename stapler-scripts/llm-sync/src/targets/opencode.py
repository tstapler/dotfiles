...
# Valid fields: description, mode, temperature, steps, disable, prompt, tools, permission, hidden, top_p
...
class OpenCodeTarget(SyncTarget, SyncSource):
    def __init__(
        self, agents_dir: Optional[Path] = None, commands_dir: Optional[Path] = None
    ):
        self.agents_dir = agents_dir or Path.home() / ".config" / "opencode" / "agents"
        self.commands_dir = (
            commands_dir or Path.home() / ".config" / "opencode" / "commands"
        )
    ...
    def load_skills(self) -> List[Skill]:
        ...
        if self.agents_dir.exists():
            for agent_file in self.agents_dir.glob("**/*.md"):
                ...
                if skill and (
                    is_in_skills_dir or skill.metadata.get("mode") == "skill"
                ):
    ...
    def save_agents(
        ...
        for agent in agents:
            ...
            if "max_steps" in agent.metadata:
                frontmatter["steps"] = agent.metadata["max_steps"]
