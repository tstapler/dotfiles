...
```yaml
# Port configuration
# NEVER use 2222 or any obviously SSH-looking port — scanners try those first.
# Leave empty; escutcheon init generates a random port (10000–60000) and stores
# it in 1Password under Escutcheon/<hostname>. Ansible fetches it at playbook time
# via the onepassword lookup. Only set this explicitly to override a stored value.
ssh_bastion_port: ""   # resolved from 1Password: lookup('community.general.onepassword', 'Escutcheon/' + inventory_hostname, field='ssh_port') | int
...
# NEVER commit a static knock sequence. Leave empty; escutcheon init generates
# 3 random ports and stores them in 1Password under Escutcheon/<hostname>.
# Ansible fetches at playbook time via onepassword lookup (see Story A.1.2a).
# Local fallback (no op CLI): ~/.local/share/escutcheon/<hostname>.json
ssh_bastion_knock_sequence: []          # resolved from 1Password — do not set statically
...
# Authorized keys
# ssh_bastion_ansible_pubkey: dedicated ed25519 key for this host only — not the main SSH key.
# Generate with: ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519_<hostname> -C "ansible@<hostname>"
# Store both halves in 1Password. Pass only the pubkey here (never the private key).
ssh_bastion_ansible_pubkey: ""          # ed25519 pubkey for headless Ansible — REQUIRED, host-specific
...
- File content (variables resolved at deploy time or templated by cfgcaddy):
  ...
  Host bastion
    ...
    User tyler
    # Port is random, generated by escutcheon init and stored in 1Password.
    # NEVER use 2222 — it is scanned. Resolve from 1Password and set here
    # as a gitignored local override; do not commit the port number.
    Port {{ bastion_ssh_port }}
    # Dedicated key for this host only — NOT the main identity.
    # Generated once: ssh-keygen -t ed25519 -f ~/.ssh/id_ed25519_bastion
    # Both halves stored in 1Password; private key never leaves the Mac.
    IdentityFile ~/.ssh/id_ed25519_bastion
    IdentitiesOnly yes
    # Explicitly disable agent fallback — do not touch the existing SSH agent.
    IdentityAgent none
...
- Note: `bastion_hostname` and `bastion_ssh_port` must be supplied via cfgcaddy vars or a gitignored local override; do not commit either value
- Note: `IdentityAgent none` is intentional — this host uses a dedicated key and must not fall through to the 1Password agent or any other running agent
- Note: no YubiKey/FIDO2 entries here — those are for interactive sessions via the main agent; the bastion dedicated key is plain ed25519 for Ansible compatibility
...
**Story D.0** — `escutcheon init` subcommand — generate and store secrets

One-time setup command run before the first Ansible playbook. Generates the SSH
port and knock sequence, stores them in 1Password, and outputs the config snippet
for `~/.config/escutcheon/config.toml`.

```
escutcheon init --host <hostname> [--op-vault <vault>] [--op-account <account>]
```

Logic:
1. Check 1Password for existing item `Escutcheon/<hostname>` via `op item get`
2. If not found (first run):
   - Generate 3 random knock ports in range 10000–60000, avoiding well-known ports
   - Generate 1 random SSH port in same range (must differ from knock ports)
   - Store to 1Password as a new Login item under `Escutcheon/<hostname>` with fields:
     - `knock_sequence`: comma-separated string e.g. `"43817,29054,61203"`
     - `ssh_port`: integer as string e.g. `"47291"`
   - If `op` CLI unavailable: write to `~/.local/share/escutcheon/<hostname>.json` (mode 0600)
3. Print TOML snippet for `~/.config/escutcheon/config.toml`:
   ```toml
   [hosts.<hostname>]
   address  = "<hostname>"
   ssh_port = 47291
   sequence = [43817, 29054, 61203]
   ```
4. Print Ansible reminder: set `ssh_bastion_port` and `ssh_bastion_knock_sequence` via
   `community.general.onepassword` lookup — never from static vars

Port generation constraints:
- Exclude: 22, 80, 443, 8080, 8443, 2222, and any port < 10000
- Ensure all 4 generated ports (3 knock + 1 SSH) are distinct
- Use `OsRng` (not pseudo-random) for cryptographic unpredictability

Ansible integration (Story A.1.2a — to be added):
```yaml
# host_vars/<hostname>.yml — no secrets here
ssh_bastion_knock_sequence: >-
  {{ lookup('community.general.onepassword', 'Escutcheon/' + inventory_hostname,
     field='knock_sequence', vault='Personal') | split(',') | map('int') | list }}
ssh_bastion_port: >-
  {{ lookup('community.general.onepassword', 'Escutcheon/' + inventory_hostname,
     field='ssh_port', vault='Personal') | int }}
```

Dependencies: `clap` (existing), `rand` + `rand_core` with `OsRng`, `serde_json` (for local fallback)
