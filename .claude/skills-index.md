...
| `go-depguard-architecture` | Configuring depguard in golangci-lint to enforce hexagonal / clean architecture import direction in Go; preventing domain packages from importing adapters or infrastructure; diagnosing and fixing layer violations; setting up tiered architecture linter rules | "Add depguard rules for hexagonal architecture", "Enforce domain can't import server", "Configure linter for clean architecture tiers", "Fix depguard violations" |
| `sqlite-optimizer` | Expert SQLite database optimization, schema design review, and PRAGMA configuration for SQLite-backed applications; WAL/journal mode, write contention, ent ORM query analysis, index strategy, type affinity mismatches, limited ALTER TABLE migration patterns | "Why is my SQLite app getting SQLITE_BUSY?", "Optimize this ent schema", "Should I use WAL mode?", "Review this SQLite index strategy", "Diagnose slow queries in SQLite" |
...
| `zombie-debugger` | Diagnosing and resolving zombie process accumulation; measuring leak rate; identifying cmd.Start() without Wait; killing residual zombies; validating a zombie fix | "Why are there zombie processes?", "Are zombies still accumulating?", "How do I reap zombie children?", "Validate this zombie fix" |
...
| Skill | Invoke When | Examples |
|-------|------------|----------|
| `eng-design-review` | End-to-end engineering design doc / RFC review: fetch Confluence page, analyze against three-layer onion framework, draft tiered inline comments (🔴/🟡/🔵), pause for user review, post approved comments, surface open threads needing response | "Review this RFC", "Post my design doc comments", "Respond to open threads on this Confluence page" |
...
| `browser-profiling` | Diagnosing browser/React/JavaScript slowness; capturing Chrome performance traces with Playwright; identifying long tasks, React re-render cascades, layout thrashing, memory leaks, and oversized bundles; fix verification via numeric baseline comparison | "App is slow", "React is re-rendering too much", "Page feels janky", "Memory keeps growing", "Profile this interaction" |
...
| `hooker` | Designing, writing, and managing Claude Code hooks; scanning conversation history to identify hook candidates; classifying controls by layer (Protection/Quality/Workflow/Intelligence); generating hookify-compatible rule files with correct dedup strategy | "What hooks should I have?", "Write a hook to block rm -rf", "Why is my hook firing twice?", "Add dedup to this hook script" |
| `session-compare` | Comparing two Claude Code sessions by metrics: duration, model, all token buckets, estimated cost, human turns, tool calls, files touched, markdown conversation summary | "Compare these two sessions", "What did each approach cost?", "Show me session metrics for these two transcripts", "Generate a comparison report" |
