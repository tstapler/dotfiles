...
// validateAccountInput validates the inputs for account creation.
func validateAccountInput(username, email string, age int) error {
	if username == "" {
...
func CreateAccount(username, email string, age int) (string, error) {
	if err := validateAccountInput(username, email, age); err != nil {
		return "", err
	}
