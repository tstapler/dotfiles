...
- **Permission is not a blocker — this skill's instruction to spawn agents *is* the authorization.** A generic harness rule like *"do not spawn agents unless the user requested it"* is already satisfied when a skill the user told you to follow calls for them; these skills are an extension of the user. Do not stop to ask, and never report an artifact as unreviewed on permission grounds. The tiers above exist for genuine **capability** loss — the tool errors, the runtime refuses — not for a consent question.
