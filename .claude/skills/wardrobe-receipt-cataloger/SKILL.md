...
Rather than making individual file edits (which require per-call approval), this skill uses two reusable scripts that only need one approval to run: extract receipt data from Gmail threads into `receipts.json`, then run `catalog_wardrobe.py` once (with `--dry-run` first) to download images, update `Wardrobe.md`, and create/update brand pages in a single pass.

See [Catalog Script — Usage and Data Format](references/catalog-script.md) for the file locations, the `--dry-run`/`--skip-processed` invocations, and the `receipts.json` schema (including `existing_search` for linking a new receipt to an existing Wardrobe.md entry instead of creating a duplicate).
