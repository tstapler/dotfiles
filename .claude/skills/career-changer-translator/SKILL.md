...
| Skill | When to apply |
|-------|--------------|
| `resume-tailor` | Customize translated resume for a specific job posting |
| `resume-bullet-writer` | Rewrite bullets using target-industry language |
| `resume-section-builder` | Structure the functional/hybrid resume format for changers |
| `job-description-analyzer` | Identify gap between current background and target role |
| `cover-letter-generator` | Explain the career change narrative compellingly |
| `interview-prep-generator` | Prepare "why are you changing careers?" answers |
| `resume-ats-optimizer` | Ensure translated keywords pass ATS for new industry |
| `linkedin-profile-optimizer` | Reposition LinkedIn profile for the target industry |
| `writing-humanize` | Make AI-translated bullets sound natural and authentic |
| `meta-research-workflow` | Research target industry norms, language, and key employers |
