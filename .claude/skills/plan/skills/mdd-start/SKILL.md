---
description: Run ideation interview and create requirements.md to start a new Manifest-Driven
  Development project
prompt: '# MDD Project Start: Ideation → requirements.md


  You are starting a new Manifest-Driven Development (MDD) project. This command runs
  the ideation phase (Phase 1) and produces `requirements.md` as its output artifact.
  ...
  If `{{args}}` was provided, use it as the project name (normalized to lowercase-kebab-case).
  Otherwise, derive it from the conversation or ask.
  ...
  **Round 2 — Scope & constraints:**
  ...
  - "What are the must-have capabilities (MoSCoW Must)? What''s explicitly out of
  scope?"

  - "What constraints exist — tech stack, timeline, team size, dependencies on other
  systems?"
  ...
  **Proceed to research?** (or open a fresh session and run `/research-workflow` with
  ...
  - If the user already has research, confirm which dimensions are covered before
  spawning agents
