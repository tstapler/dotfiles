...
| Skill | When to apply |
|-------|--------------|
| `resume-bullet-writer` | Sharpen strategic achievement bullets for each role |
| `resume-quantifier` | Add P&L scale, team size, and revenue metrics |
| `resume-formatter` | Ensure clean 2-3 page layout appropriate for executive level |
| `cover-letter-generator` | Write executive-level board or search firm letters |
| `interview-prep-generator` | Prepare leadership philosophy and transformation stories |
| `linkedin-profile-optimizer` | Align LinkedIn presence for executive recruiter visibility |
| `salary-negotiation-prep` | Negotiate total compensation including equity and bonuses |
| `offer-comparison-analyzer` | Compare C-suite offers across total comp and strategic fit |
| `resume-tailor` | Adjust emphasis for CEO vs. COO vs. board roles |
| `pdf-proof` | Proofread before submitting to executive search firms |
| `writing-humanize` | Ensure AI-drafted executive profile language sounds genuine |
