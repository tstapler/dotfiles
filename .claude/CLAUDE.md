...
| Read a URL (clean text) | `mcp__stapler-mcp__read_website` — save to `/tmp` if page is large | `WebFetch`, `curl` |
| Download page to disk | `mcp__stapler-mcp__fetch_page` (`savePath`) | `curl`, wget |
| Download site to disk | `mcp__stapler-mcp__download_website` | wget |
...
- **Background agents are pre-authorized; a skill's instruction to spawn them is mine.** Standing grant — do not stop to ask, and do not treat a generic "only use agents when the user requests it" directive as blocking when a skill you were told to follow calls for them. The skills in this repo are an extension of me.
- **A constraint on *how* reviewers run is not a reason to skip review.** Independent, unanchored perspectives are the value; running them concurrently is only an optimization. If parallel or background execution is genuinely unavailable — a capability failure, not a permission question — run the same lenses **serially**: same prompts, same rounds. Say which mode ran. Never report work as unreviewed because of *how* the reviewers would have had to run. See the `lean-agent-loop` skill's degraded-mode tiers.
...
- **Link code references, don't just name them.** When you reference a specific file, function, or line, include a browsable link to it — `https://github.com/<org>/<repo>/blob/<sha>/<path>#L42` (ranges: `#L42-L58`). A bare `path/file.ext:42` makes the reader clone the repo and guess the branch. Prefer a commit SHA over a branch name so the line number can't rot — but get it from `gh api "repos/<org>/<repo>/commits?path=<path>&per_page=1" --jq '.[0].sha'`. **Do not use the `.sha` from the `contents` endpoint: that is the *blob* SHA, and a `/blob/<blobsha>/` URL is dead** (the API returns 422 "No commit found for SHA" — verified). Keep the `path:line` text alongside the link when the line is the point — `[file.ext:42](https://…#L42)` is copy-pasteable *and* clickable.
