...
| `tymuxd` | Ported, no sudo needed — run live twice on this machine (tymuxd already on PATH via an unrelated dev symlink, correctly detected as present via the ENOENT-vs-other-exit-code check both times, zero operations queued either run). The actual fetch-and-symlink install path is code-review only — this machine already has tymuxd, so it's never been exercised for real; needs verification on a machine without a stapler-squad tymuxd build | `deploys/tymuxd.py` |
| `ssh-bastion-client` | Ported, no sudo needed — run live twice on this machine with the default empty `bastion_op_item` (no bastion configured here), correctly no-op both times (zero operations queued). The configured path (`op read`, keygen, SSH config + escutcheon.toml rendering) is code-review + `test_ssh_bastion_client.py` only — deliberately not triggered for real without a `bastion_op_item` set, since it writes real SSH config/keys and queries 1Password (same caution as `fbg`) | `deploys/ssh_bastion_client.py` |
| `memory-optimizer` (no Ansible-role equivalent in `bootstrap/` — lives in `stapler-scripts/roles/memory-optimizer`, invoked from `bootstrap/playbook.yml`) | Ported, NOT run live anywhere — every task needs root (swapfile/zswap/GRUB/systemd/sysctl), and this session has no real terminal for the sudo prompts. Code review + the GRUB-cmdline regex/replacement pure-logic check in `test_memory_optimizer.py` are the verification so far. Needs a real terminal to run live and confirm idempotency before removing from `playbook.yml` | `deploys/memory_optimizer.py` |
